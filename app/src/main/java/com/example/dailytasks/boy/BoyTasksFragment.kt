...
class BoyTasksFragment : Fragment(R.layout.fragment_tasks) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity as AppCompatActivity).supportActionBar?.title = "Редактирование списка задач - Boy"
        ...
        deleteButton.setOnClickListener {
            dailyTasksViewModel.deleteBoyTasksList()
            linearLayout.removeAllViewsInLayout()
            categoryID = 1
        }

        saveButton.setOnClickListener {
            findNavController().navigate(R.id.action_tasksFragment_to_dailyTasksFragment)
        }
        ...
        fun renameDialog(textView: TextView) {
            ...
            dailyTasksViewModel.sortBoyTaskListByName(textView.text.toString())?.observe(viewLifecycleOwner) { taskInDB ->
                for (i in taskInDB) {
                    with(builder) {
                        setTitle("Rename")
                        setPositiveButton("OK") { _, _ ->
                            textView.text = editText.text
                            dailyTasksViewModel.updateBoyTaskList(BoyTasksList(i.number, editText.text.toString(), i.viewType))
                        }
        ...
        fun editCategoryDialog(textView: TextView) {
            val builder = AlertDialog.Builder(context)
                ...
                .setPositiveButton("Delete") { _, _ ->
                    ...
                    dailyTasksViewModel.deleteBoyTask(textView.text.toString())
            ...
            with(builder) {
                ...
                setPositiveButton("OK") { _, _ ->
                    ...
                    if (textView.text.isNotEmpty()) {
                        dailyTasksViewModel.insertBoyTaskList(BoyTasksList(
                            categoryID,
