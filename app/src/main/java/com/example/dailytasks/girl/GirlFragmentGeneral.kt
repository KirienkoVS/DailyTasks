...
class GirlFragmentGeneral : Fragment(R.layout.fragment_girl_general) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity as AppCompatActivity).supportActionBar?.title = "Список результатов по дням - Girl"
        ...
        dailyTasksViewModel.getGirlResults.observe(viewLifecycleOwner) { resultsDB ->
            ...
            val adapter = GirlRecyclerAdapter(resultsList)
            binding.recyclerView.adapter = adapter
        }

        binding.fab.setOnClickListener {
            findNavController().navigate(R.id.action_girlFragmentGeneral_to_girlFragment)
        }
    }
