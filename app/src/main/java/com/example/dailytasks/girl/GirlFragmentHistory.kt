...
const val GIRLDATE = "date"

class GirlFragmentHistory : Fragment(R.layout.fragment_boy_history) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity as AppCompatActivity).supportActionBar?.title = "Просмотр выполненных задач - Girl"

        if (savedInstanceState != null) {
            binding.date.text = savedInstanceState.getCharSequence(GIRLDATE)
        }
        ...
        //DRAWING THE LAYOUT
        dailyTasksViewModel.sortGirlTasksTableByDate(dateID)?.observe(viewLifecycleOwner) { sortedList ->
        ...
        dailyTasksViewModel.sortGirlResultByDate(dateID)?.observe(viewLifecycleOwner) { resultsDB ->
            for (i in resultsDB)
                resultID.add(i.id)
        }
        ...
        binding.updateButton.setOnClickListener {
            ...
            dailyTasksViewModel.updateGirlResult(GirlResultsTable(resultID[0], dateID, dayResult(), rating()))
            findNavController().navigate(R.id.action_girlFragmentHistory_to_girlFragmentGeneral)
        }
    }

    override fun onSaveInstanceState(outState: Bundle) {
        super.onSaveInstanceState(outState)
        outState.putCharSequence(GIRLDATE, fragmentBinding?.date?.text)
    }

    override fun onDestroyView() {
        super.onDestroyView()
        fragmentBinding = null
        Log.d("girl_history", "onDestroyView() is called")
    }
}